...
import { Package, ShoppingCart, LogOut, X } from "lucide-react";
...
export default function AdminSidebar({ onLogout, open, onClose }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:relative z-50 h-full w-[240px] bg-white border-r border-[var(--candy-border)] p-4 flex flex-col transition-transform duration-200 ${
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={onClose}
            className="p-1 rounded-lg hover:bg-[#F1F5F9] transition-colors md:hidden"
          >
            <X className="size-4 text-[var(--candy-muted)]" />
          </button>
        </div>
